...
export default class NewPage {
  ...
  async afterRender() {
    ...
    // Add cleanup handler for page transitions
    window.addEventListener('hashchange', this.#handlePageTransition.bind(this));
    this.#cleanupFunctions.push(() => {
      window.removeEventListener('hashchange', this.#handlePageTransition.bind(this));
    });
  }
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Tambahkan popup pada marker saat pertama kali dibuat
          this.#marker.bindPopup(
            `<b>Lokasi Awal</b><br>Latitude: ${latitude.toFixed(6)}<br>Longitude: ${longitude.toFixed(6)}`,
            { autoClose: false, closeOnClick: false }
          ).openPopup();

          // Event klik pada peta untuk mengubah posisi marker dan memperbarui popup
          this.#map.on("click", (event) => {
            ...
            this.#marker.bindPopup(
              `<b>Lokasi Baru</b><br>Latitude: ${lat.toFixed(6)}<br>Longitude: ${lng.toFixed(6)}`,
              { autoClose: false, closeOnClick: false }
            ).openPopup();
